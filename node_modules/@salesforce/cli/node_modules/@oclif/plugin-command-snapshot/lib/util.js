 ...
 */
export const getSchemaFileName = (cmdId) => {
 ...
 */
export const getKeyNameFromFilename = (file) => file.replaceAll('-', ':').replaceAll('__', '-').replace('.json', '');
export const getAllFiles = (dirPath, ext, allFiles = []) => safeReadDirSync(dirPath)
    .flatMap((f) => f.isDirectory() ? getAllFiles(path.join(dirPath, f.name), ext, allFiles) : path.join(dirPath, f.name))
