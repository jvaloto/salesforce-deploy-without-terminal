...
class CodeCoverage {
    ...
    async queryAggregateCodeCov(apexClassIdSet) {
        ...
        if (apexClassIdSet.size === 0) {
            const query = 'SELECT ApexClassOrTrigger.Id, ApexClassOrTrigger.Name, NumLinesCovered, NumLinesUncovered, Coverage FROM ApexCodeCoverageAggregate';
            const result = await (0, utils_1.queryAll)(this.connection, query, true);
            return [result];
        }
        // If the "Store Only Aggregate Code Coverage" setting is unchecked, we continue to query only the Apex classes and triggers in apexClassIdSet from the ApexCodeCoverageAggregate table, as those are the Apex classes and triggers touched by the Apex tests in the current run.
            ...
            const query = 'SELECT ApexClassOrTrigger.Id, ApexClassOrTrigger.Name, NumLinesCovered, NumLinesUncovered, Coverage FROM ApexCodeCoverageAggregate WHERE ApexClassorTriggerId IN (%s)';
    ...
    async fetchResults(idSet, selectQuery) {
        ...
        (0, utils_1.queryAll)(this.connection, query, true));
    ...
    createQueries(selectQuery, idSet) {
        const queries = [];
        for (let i = 0; i < idSet.size; i += constants_1.QUERY_RECORD_LIMIT) {
