...
import { makeFlags, promptForFlag, validateAgentType, validateMaxTopics, validateTone, validateAgentUser, } from '../../../flags.js';
    ...
    if (!noPrompt) {
        if (existsSync(resolvedOutputFile)) {
            ...
            if (!(await prompts.confirm({ message }))) {
                return resolveOutputFile(await promptForFlag({
                    message: messages.getMessage('flags.output-file.summary'),
                    validate: (d) => d.length > 0 || 'Output file cannot be empty',
                }));
            }
        }
    }
