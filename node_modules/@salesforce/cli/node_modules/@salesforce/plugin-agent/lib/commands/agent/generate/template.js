...
const convertBotToBotTemplate = (bot, botVersionJson, newMlDomainName, botFilePath) => {
    ...
    if (!entryDialogJson)
        throw new SfError(messages.getMessage('error.no-entry-dialog'));
    ...
    if (bot.Bot.type === 'Bot')
        throw new SfError(messages.getMessage('error.invalid-bot-type', [botFilePath]));
    if (!bot.Bot.label)
        throw new SfError(messages.getMessage('error.no-label', [botFilePath]));
    if (!bot.Bot.botMlDomain)
        throw new SfError(messages.getMessage('error.no-ml-domain', [botFilePath]));
