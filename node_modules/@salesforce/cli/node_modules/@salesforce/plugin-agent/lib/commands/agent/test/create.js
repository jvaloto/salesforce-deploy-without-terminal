...
async function promptUntilUniqueName(connection, name) {
...
export default class AgentTestCreate extends SfCommand {
    ...
    async run() {
        ...
        if (flags.preview) {
            ...
            this.log(messages.getMessage('info.success', [path, flags['target-org'].getUsername() ?? flags['target-org'].getOrgId()]));
        }
        return {
            path,
            contents,
