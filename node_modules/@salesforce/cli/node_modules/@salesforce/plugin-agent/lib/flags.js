...
export const promptForAiEvaluationDefinitionApiName = async (flagDef, connection) => {
...
export const validateAgentUser = async (connection, agentUser) => {
    if (agentUser?.length) {
        try {
            const q = `SELECT Id FROM User WHERE Username = '${agentUser}'`;
            await connection.singleRecordQuery(q);
        }
        catch (error) {
...
export const getAgentUserId = async (connection, agentUser) => {
    const q = `SELECT Id FROM User WHERE Username = '${agentUser}'`;
    return (await connection.singleRecordQuery(q)).Id;
};
