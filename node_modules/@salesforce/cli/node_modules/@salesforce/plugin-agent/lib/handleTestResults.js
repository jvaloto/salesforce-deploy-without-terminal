...
import { convertTestResultsToFormat, humanFriendlyName, metric } from '@salesforce/agents';
...
export function readableTime(time, decimalPlaces = 2) {
    if (time < 1000) {
        return '< 1s';
    }
...
export function humanFormat(results) {
    const ux = new Ux();
    const tables = [];
    const metricResults = [];
    for (const testCase of results.testCases) {
        let table = ux.makeTable({
            ...
            data: testCase.testResults
                ...
                result: r.result === 'PASS' ? ansis.green('Pass') : r.status === 'ERROR' ? ansis.red('Error') : ansis.red('Fail'),
        ...
        const metrics = testCase.testResults.filter((f) => metric.includes(f.name));
        if (metrics.length > 0) {
            ...
            table = ux.makeTable({
                ...
                data: metrics.map((r) => ({
                    ...
                    result: r.result === 'PASS' || r.name === 'output_latency_milliseconds' ? ansis.green('Pass') : ansis.red('Fail'),
    ...
    const metricPassCount = metricResults.filter((f) => f.result === 'PASS' || f.name === 'output_latency_milliseconds').length;
