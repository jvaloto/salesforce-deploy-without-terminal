...
import { Flags, optionalOrgFlagWithDeprecations, loglevel } from '@salesforce/sf-plugins-core';
...
export async function bulkIngest(opts) {
    ...
    const columnDelimiter = opts.columnDelimiter ?? (['delete', 'hardDelete'].includes(operation) ? 'COMMA' : await detectDelimiter(file));
    ...
    try {
        ...
        if (jobInfo.numberRecordsFailed) {
            stages.error();
            throw messages.createError('error.failedRecordDetails', [jobInfo.numberRecordsFailed], [conn.getUsername(), job.id, conn.getUsername(), job.id]);
        }
        stages.stop();
        return {
            jobId: jobInfo.id,
        ...
        if (jobInfo.state === 'Failed') {
            stages.error();
            throw messages.createError('error.jobFailed', [jobInfo.errorMessage], [conn.getUsername(), job.id, conn.getUsername(), job.id]);
        }
    ...
    try {
        ...
        if (jobInfo.numberRecordsFailed) {
            ...
            if (['delete', 'hardDelete', 'upsert'].includes(jobInfo.operation) && opts.jsonEnabled) {
                opts.warnFn('Record failures will not be included in JSON output after March 2025, use `sf data bulk results` to get results instead.');
                return {
                    jobId: jobInfo.id,
                    processedRecords: jobInfo.numberRecordsProcessed,
                    successfulRecords: jobInfo.numberRecordsProcessed - (jobInfo.numberRecordsFailed ?? 0),
        ...
        if (jobInfo.state === 'Failed') {
            stages.error();
            throw messages.createError('error.jobFailed', [jobInfo.errorMessage], [conn.getUsername(), job.id, conn.getUsername(), job.id], err);
        }
        if (jobInfo.state === 'Aborted') {
            stages.stop('aborted');
            throw messages.createError('error.jobAborted', [], [conn.getUsername(), job.id, conn.getUsername(), job.id], err);
        }
        throw err;
    }
}
