...
import { ConfigAggregator, EnvironmentVariable, Lifecycle, Logger, Messages, OrgConfigProperties, SfError, SfProject, } from '@salesforce/core';
import { RetrieveResult, ComponentSetBuilder, ComponentSet, RegistryAccess, RequestStatus, ComponentStatus, } from '@salesforce/source-deploy-retrieve';
...
export default class RetrieveMetadata extends SfCommand {
    ...
    async moveResultsForRetrieveTargetDir(targetDir, resolvedTargetDir) {
        ...
        if (this.retrieveResult.getFileResponses().length ===
...
const hasRootTypesWithDependencies = (flags, apiVersion) => {
    const hasDeps = !!flags.metadata?.some(isPseudoType) && Number(apiVersion) > 63.0;
    if (hasDeps) {
        getLogger().debug('Requesting metadata with rootTypesWithDependencies param because a pseudotype was detected and API Version is 64.0 or higher.');
    }
    return hasDeps;
};
// Resolve the API Version used for the retrieve. NOTE: it does not resolve sourceApiVersion.
const resolveApiVersion = async (flags) => {
    try {
