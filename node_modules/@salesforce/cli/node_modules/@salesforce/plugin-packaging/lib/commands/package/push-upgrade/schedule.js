...
export class PackagePushScheduleCommand extends SfCommand {
    ...
    async run() {
        ...
        const result = await PackagePushUpgrade.schedule(conn, flags.package, startTime, orgList, isMigration);
        this.log(messages.getMessage('output', [result?.PushRequestId, orgList.join(', ')]));
        return result;
    }
}
async function readOrgFile(filePath) {
    try {
        const fileContent = await fs.readFile(filePath, 'utf-8');
        const orgIds = fileContent.split(/\r?\n/).filter((id) => id.trim().length > 0);
        return orgIds.filter((id) => /^00D[a-zA-Z0-9]{12}$/.test(id));
    }
    catch (error) {
        throw new SfError(messages.getMessage('error.invalid-org-file'));
    }
}
function getOrgListFromInput(orgInput) {
    try {
