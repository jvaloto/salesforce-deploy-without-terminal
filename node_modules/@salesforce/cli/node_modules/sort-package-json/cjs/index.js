...
var sortScripts = onObject((scripts, packageJson) => {
  ...
  const scriptsKeyMap = /* @__PURE__ */ new Map();
  keys.flatMap(
    (key) => prefixable.has(key) ? [`pre${key}`, key, `post${key}`] : [key]
  ).forEach((key) => {
    const [prefix] = key.split(":");
    const keySet = scriptsKeyMap.has(prefix) ? scriptsKeyMap.get(prefix) : /* @__PURE__ */ new Set();
    scriptsKeyMap.set(prefix, keySet.add(key));
  });
  const order = [...scriptsKeyMap.values()].flat().reduce((keys2, keySet) => {
    keys2.push(...keySet);
    return keys2;
  }, []);
